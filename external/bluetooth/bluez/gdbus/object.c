...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_GDBUS_OBJECT_C
							...
							void *iface_user_data)
{
    ...
    DBG("*** gdbus(object.c) process_message() method->name=%s. ***", method->name);
						...
						const char *name)
{
	...
	for (list = interfaces; list; list = list->next) {
		struct interface_data *iface = list->data;
		DBG("*** gdbus(object.c) find_interface() iface->name=%s name=%s. ***", iface->name, name);
					...
					DBusMessage *message, void *user_data)
{
    ...
    DBG("*** gdbus(object.c) generic_message() interface=%s. ***", interface);
	...
	for (method = iface->methods; method &&
		...
		DBG("*** gdbus(object.c) generic_message() method->name=%s. ***",method->name);
							...
							const char *path)
{
    ...
    DBG("*** gdbus(object.c)  object_path_ref() path = %s. ***", path);
						...
						va_list var_args)
{
	...
	if (signal == NULL) {
		...
		DBG("*** Unable to allocate new %s.%s signal ***", interface,  name);
		return FALSE;
	}

	DBG("*** dbus_message_new_signal() OK %s.%s signal ***", interface,  name);
	...
	if (strcmp(args, signature) != 0) {
		...
		DBG("*** %s.%s: expected signature'%s' but got '%s' ***",
				interface, name, args, signature);
		ret = FALSE;
		goto fail;
	}

    DBG("*** %s.%s: dbus_message_get_signature() '%s' but got '%s' ***",
				interface, name, args, signature);

	ret = dbus_connection_send(conn, signal, NULL);
    DBG("*** dbus_connection_send(() OK ret=%d ***", ret);
					...
					GDBusDestroyFunction destroy)
{
	...
	if (find_interface(data->interfaces, name)) {
		...
		DBG("*** ERR g_dbus_register_interface(). data->interfaces=%s, name = %s. ***",data->interfaces, name);
		return FALSE;
	}
