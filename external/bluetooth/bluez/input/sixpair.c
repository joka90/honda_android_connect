...
#include <dbus/dbus.h>
/* This must be exactly specified as there are muptiple device.h files
   and one in the same folder is not the correct one */
...
handle_device (libusb_device *dev, struct libusb_config_descriptor *cfg, int itfnum, const struct libusb_interface_descriptor *alt)
{
	...
	if (option_store_info != FALSE) {
		...
		str2ba(device, &dst);
		write_device_profiles(&src, &dst, "00001124-0000-1000-8000-00805f9b34fb");
