...
#define   SLINK_CS_ACTIVE_BETWEEN(x)	(((x) & 0x1) << 17)
		    ...
		    bool is_single_xfer)
{
	...
	if (is_first_of_msg) {
		...
		if (cdata && cdata->is_hw_based_cs && is_single_xfer) {
			dev_info(&tspi->pdev->dev, "Enabling hw chipselect, "
			    "cs_inactive_period:%d, cs_setup_clk_count:%d\n",
			    cdata->cs_inactive_period,
...
static int spi_tegra_setup(struct spi_device *spi)
{
	...
	pm_runtime_get_sync(&tspi->pdev->dev);
	tspi->def_command_reg &= ~SLINK_IDLE_SCLK_MASK & ~SLINK_CK_SDA;
	if (spi->mode & SPI_CPHA)
		tspi->def_command_reg |= SLINK_CK_SDA;

	if (spi->mode & SPI_CPOL)
		tspi->def_command_reg |= SLINK_IDLE_SCLK_DRIVE_HIGH;
	else
		tspi->def_command_reg |= SLINK_IDLE_SCLK_DRIVE_LOW;
...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	tspi->def_command2_reg = SLINK_CS_ACTIVE_BETWEEN(1);
