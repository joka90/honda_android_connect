...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_SRC_STORAGE_C
...
int write_link_key(bdaddr_t *local, bdaddr_t *peer, unsigned char *key, uint8_t type, int length)
{
	...
	ba2str(local, local_bdaddr);
	ba2str(peer, peer_bdaddr);
	
	DBG("local:%s ", local_bdaddr);
	DBG("peer:%s ", peer_bdaddr);
	DBG("link_key:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
	      key[15],key[14],key[13],key[12],
	      key[11],key[10],key[9],key[8],
	      key[7],key[6],key[5],key[4],
	      key[3],key[2],key[1],key[0]);

	BLUEZ_DEBUG_LOG("[STRAGE] bdaddr : [ %s ]", peer_bdaddr);
	BLUEZ_DEBUG_LOG("[STRAGE] link_key : [ %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x ]",
	      key[15],key[14],key[13],key[12],
	      key[11],key[10],key[9],key[8],
	      key[7],key[6],key[5],key[4],
	      key[3],key[2],key[1],key[0]);
...
int read_link_key2(bdaddr_t *local, bdaddr_t *peer, unsigned char *key)
{
	char filename[PATH_MAX + 1], *str, addr[18];
...
uint8_t count_link_key(bdaddr_t *local)
{
	char filename[PATH_MAX + 1];
	uint8_t num;
	...
	num = textfile_get_linkkey_num(filename);
