...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_AUDIO_DEVICE_C
...
void audio_device_unregister(struct audio_device *device)
{
	...
	if (device->handsfree)
		handsfree_unregister(device);

	if (device->a2dpad)
		a2dpad_unregister(device);

	if (device->avrcpad)
		avrcpad_unregister(device);

	if (device->browsing)
		browsing_unregister(device);

	if (device->pbap)
		pbap_unregister(device);
