...
static int card_detect(bool card_present, struct sdhci_host *sdhost)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(sdhost);
	struct tegra_sdhci_host *tegra_host = pltfm_host->priv;
...
static int tegra_sdhci_resume(struct sdhci_host *sdhci)
{
	...
	if (gpio_is_valid(plat->cd_gpio) && plat->cd_wakeup_incapable) {
		if (tegra_host->card_present && gpio_get_value(plat->cd_gpio))
			return card_detect(false, sdhci);
		else if (!tegra_host->card_present &&
			!gpio_get_value(plat->cd_gpio))
			return card_detect(true, sdhci);
	}
...
static int __devinit sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	tegra_host->uhs_mask = plat->uhs_mask;
