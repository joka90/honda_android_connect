...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_SRC_ADAPTER_C
...
#define ADAPTER_SDP_TIMER 10

#define check_address(address) bachk(address)

#define DABLUETOOTHD_PATH "/data/misc/dabluetoothd/reset_bluetooth"
#define BLUETOOTH_DEVICE_INVALID_ADDRESS "XX:XX:XX:XX:XX:XX"
static void remove_bluetoothdata(struct btd_adapter *adapter);
				...
				gboolean discoverable, void *data)
{
	...
	if (err < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] set_mode() ERROR");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		if ( msg != NULL ) {
							...
							gboolean pairable)
{
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Pairable ] : %d", pairable);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_PAIRABLE, pairable);
							...
							void *data)
{
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ DiscoverableTimeout ] : %d", timeout);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_DISCOVERABLETIMEOUT, timeout);
						...
						void *data)
{
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ PairableTimeout ] : %d", timeout);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_PAIRABLETIMEOUT, timeout);
	emit_property_changed(conn, path,
...
void btd_adapter_class_changed(struct btd_adapter *adapter, uint32_t new_class)
{
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Class ] : 0x%08x", new_class);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_CLASS, new_class);
	emit_property_changed(connection, adapter->path,
...
void adapter_update_local_name(struct btd_adapter *adapter, const char *name)
{
	...
	if (!adapter->name_stored) {
		...
		if (connection)
			BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Name ] : [ %s ]", name_ptr);
			EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_NAME, 0);
			emit_property_changed(connection, adapter->path,
					...
					const char *name, void *data)
{
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Name ] : [ %s ]", name_ptr);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_NAME, 1);
	emit_property_changed(connection, adapter->path,
					ADAPTER_INTERFACE, "Name",
					DBUS_TYPE_STRING, &name_ptr);
					...
					DBusMessage *msg, void *data)
{
	...
	num = count_link_key( &adapter->bdaddr );
	BLUEZ_DEBUG_WARN("[HCI] Linkkey Num = %d BDADDR = [ %s ]",  num, address);
	str2ba(address, &bdaddr);
	EVENT_DRC_BLUEZ_MANAGER(FORMAT_ID_BT_MANAGER, MANAGER_CREATE_PAIRDEVICE, &bdaddr, num);
	if ( num >= MAX_PAIRING_NUM ) {
		BLUEZ_DEBUG_WARN("Refusing pairing: MAX PAIRING OVER NUM");
		err = EUSERS;
		return btd_error_failed(msg, strerror(err));
	}
...
static DBusMessage *adapter_get_linkkey(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	DBusMessage *reply;
	struct btd_adapter *adapter = data;
	...
	bdaddr_t dst;
	uint32_t len;
	const char *c_linkkey;
	unsigned char linkkey[33];
	int cnt;
	const char *pos;
	const char key[2];

	BLUEZ_DEBUG_INFO("[ADAPTER] Start.");

	if (!dbus_message_get_args(msg, NULL,
								DBUS_TYPE_STRING, &addr,
								DBUS_TYPE_INVALID)){
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] dbus_message_get_args() ERROR");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		return NULL;
	}
	...
	memset( &dst, 0, sizeof( bdaddr_t ) );

	memcpy( &dstaddr, addr, sizeof(dstaddr) );
	if ( 0 != strcmp( dstaddr, BLUETOOTH_DEVICE_INVALID_ADDRESS ) ) {
		str2ba(dstaddr, &dst);
		ret = read_link_key2(&src, &dst, linkkey);
	} else {
		ret = read_link_key2(&src, NULL, linkkey);
	}

	if( ret != 0 ){
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] read_link_key2() ERROR");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		EVENT_DRC_BLUEZ_BDADDR(FORMAT_ID_BT_ERR_BDADDR, BLUEZ_SRC_ID_BDADDR, ERROR_BLUEZ_SRC_ADAPTER, __LINE__, &dst);
		return g_dbus_create_error(msg,
			ERROR_INTERFACE ".Failed", "Failed to get linkkeys");
	}

	c_linkkey = (const char *)malloc( 33 );
	memset( (void *)c_linkkey, '\0', 33 );

	pos = c_linkkey;
	for( cnt = 30; cnt >= 0; ) {
		memcpy( (void *)&key, &linkkey[ cnt ], 2);
		memcpy( (void *)pos, &key, 2);
		pos = pos + 2;
		cnt = cnt - 2;
	}

	BLUEZ_DEBUG_LOG("[ADAPTER] Linkkey = [ %s ]", c_linkkey);

	reply = dbus_message_new_method_return(msg);
	dbus_message_append_args(reply, DBUS_TYPE_STRING, &c_linkkey, DBUS_TYPE_INVALID);
	free( (void *)c_linkkey );

	BLUEZ_DEBUG_INFO("[ADAPTER] End.");
...
static gboolean sdp_timer_cb(gpointer data)
{
	...
	BLUEZ_DEBUG_INFO("[ADAPTER] sdp_timer_cb(). Start.");

	adapter_get_address(adapter, &src);
	bt_cancel_discovery(&src, &adapter->sdp_dst);
	g_source_remove(adapter->sdp_timer);
	adapter->sdp_timer = 0;

	BLUEZ_DEBUG_WARN("[ADAPTER] SDP Time Out Connected by Default Infomation");
	...
	g_dbus_emit_signal(connection, adapter->path,
			...
			DBUS_TYPE_UINT16, &result,
			DBUS_TYPE_UINT16, &VendorID,
			DBUS_TYPE_UINT16, &ProductID,
			DBUS_TYPE_UINT16, &Version,
			DBUS_TYPE_INVALID);
	EVENT_DRC_BLUEZ_DID(FORMAT_ID_DID, SIGNAL_DID_SDP_TIMEOUT, result, VendorID, ProductID, Version);

	BLUEZ_DEBUG_INFO("[ADAPTER] sdp_timer_cb(). End.");

	return TRUE;
}

static DBusMessage *adapter_get_did(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	struct btd_adapter *adapter = data;
	const char *addr;
	...
	if (adapter->sdp_timer != 0 ) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] SDP is already started");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		goto failed;
	}
	BLUEZ_DEBUG_LOG("[ADAPTER] Addr = [ %02x:%02x:%02x:%02x:%02x:%02x ]", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
	memcpy( &adapter->sdp_dst, addr, sizeof(bdaddr_t) );
	adapter_get_address(adapter, &src);
	sdp_uuid16_create(&uuid, PNP_INFO_SVCLASS_ID);

	err = bt_search_service(&src, &adapter->sdp_dst, &uuid,
						get_records_did_cb, adapter, NULL);

	if (err < 0) {
		BLUEZ_DEBUG_ERROR(" [Bluez - ADAPTER] bt_search_service() ERROR. err = %d.", err);
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, err, 0);
		goto failed;
	}

	adapter->sdp_timer = g_timeout_add_seconds(ADAPTER_SDP_TIMER, sdp_timer_cb, adapter);
...
failed:
	...
	g_dbus_emit_signal(conn, adapter->path,
			...
			DBUS_TYPE_UINT16, &Version,
			DBUS_TYPE_INVALID);
	EVENT_DRC_BLUEZ_DID(FORMAT_ID_DID, SIGNAL_DEVICEIDENTIFICATION, result, VendorID, ProductID, Version);

	BLUEZ_DEBUG_INFO("[ADAPTER] Error End.");

	return dbus_message_new_method_return(msg);

}

static DBusMessage *adapter_set_did(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	uint16_t VendorID = 0;
	uint16_t ProductID = 0;
	...
	BLUEZ_DEBUG_LOG("[ADAPTER] VendorID = 0x04x ProductID = 0x04x Version = 0x04x", VendorID, ProductID, Version);
	register_device_id(VendorID, ProductID, Version);
...
static GDBusMethodTable adapter_methods[] = {
	...
	{ "GetLinkkey",	"s",	"s",	adapter_get_linkkey	},
	{ "GetDID",	"ay",	"",	adapter_get_did,	0,	0 },
	{ "SetDID",	"qqq",	"",	adapter_set_did,	0,	0 },
	{ }
};
...
static GDBusSignalTable adapter_signals[] = {
	...
	{ "DeviceFound",		"sa{sv}"	},
	{ "DeviceDisappeared",		"s"		},
	{ "DeviceIdentification",	"qqqq",	G_DBUS_SIGNAL_FLAG_DEPRECATED },
	{ }
};
...
void btd_adapter_start(struct btd_adapter *adapter)
{
	...
	powered = TRUE;
	BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Powered ] : %d", powered);
	EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_POWERED, powered);
	emit_property_changed(connection, adapter->path,
...
int btd_adapter_stop(struct btd_adapter *adapter)
{
	...
	if (adapter->scan_mode == (SCAN_PAGE | SCAN_INQUIRY)) {
		discoverable = FALSE;
		BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Discoverable ] : %d", discoverable);
		EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_DISCOVERABLE, discoverable);
		emit_property_changed(connection, adapter->path,
...
void adapter_set_state(struct btd_adapter *adapter, int state)
{
	...
	case STATE_IDLE:
		...
		discov_active = FALSE;
		BLUEZ_DEBUG_LOG("[ADAPTER] PROPERTY CHANGED [ Discovering ] : %d", discov_active);
		EVENT_DRC_BLUEZ_ADAPTER(FORMAT_ID_BT_ADAPTER, ADP_CHG_DISCOVERING, discov_active);
...
static void remove_bluetoothdata(struct btd_adapter *adapter)
{
	...
	BLUEZ_DEBUG_INFO("[ADAPTER] remove_bluetoothdata() Start.");

	ret = textfile_isexist(DABLUETOOTHD_PATH);
	if( ret != 1 ) {
		BLUEZ_DEBUG_INFO("[ADAPTER] remove_bluetoothdata() End. [ It is not necessary to reset. ]");
		return;
	}

	adapter_get_address(adapter, &bdaddr);
	ba2str(&bdaddr, srcaddr);
	create_path(filename, PATH_MAX, STORAGEDIR, srcaddr);

	/* Delete the bluetooth data file from storage */
	ret = recursive_dir_clear(filename);
	if( ret != 0 ) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] cannot remove bluetooth data files");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
	}

	/* Delete the reset info file */
	ret = recursive_dir_clear(DABLUETOOTHD_PATH);
	if( ret != 0 ) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] cannot remove the reset info file");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
	}

	BLUEZ_DEBUG_INFO("[ADAPTER] remove_bluetoothdata() End.");
...
static void get_records_did_cb(sdp_list_t *recs, int err, gpointer user_data)
{
	...
	if( adapter == NULL ) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] adapter == NULL.");
	...
	if (err < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] Unable to get service record: %s (%d)",strerror(-err), -err);
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, -err, 0);
		goto done;
	}

	if (!recs || !recs->data) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] No records found");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		goto done;
	}
	...
	for (r = recs; r != NULL; r = r->next) {
		...
		if (sdp_get_service_classes(record, &classes) < 0) {
			BLUEZ_DEBUG_LOG("[ADAPTER] Unable to get service classes from record");
			ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
			continue;
		}

		if( classes == NULL ){
			BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] sdp_get_service_classes is failed");
			ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
			goto done;
		}
		...
		if (sdp_uuid_cmp(&class, &uuid) == 0) {
			...
			nVendorID = pdlist ? pdlist->val.uint16 : 0x0000;

			pdlist = sdp_data_get(record, SDP_ATTR_PRODUCT_ID);
			nProductID = pdlist ? pdlist->val.uint16 : 0x0000;

			pdlist = sdp_data_get(record, SDP_ATTR_VERSION);
			nVersion = pdlist ? pdlist->val.uint16 : 0x0000;
			break;
		}
	}

	if (r == NULL) {
		BLUEZ_DEBUG_ERROR("[Bluez - ADAPTER] No record found with UUID 0x%04x", PNP_INFO_SVCLASS_ID);
		ERR_DRC_BLUEZ(ERROR_BLUEZ_SRC_ADAPTER, 0, 0);
		goto done;
	}

	result = TRUE;
	VendorID = ( uint16_t )nVendorID;
	ProductID = ( uint16_t )nProductID;
	Version = ( uint16_t )nVersion;
	BLUEZ_DEBUG_INFO("[ADAPTER] VendorID = 0x%04x ProductID = 0x%04x Version = 0x%04x", VendorID, ProductID, Version);
...
void btd_adapter_signal_did(int result, uint16_t vendor, uint16_t product, uint16_t version)
{
	struct btd_adapter *adapter;
	GSList *l;
	...
	for (l = manager_get_adapters(); l != NULL; l = g_slist_next(l)) {
		...
		BLUEZ_DEBUG_LOG("[ADAPTER] result = %d vendor = 0x%04x product = 0x%04x version = 0x%04x", result, vendor, product, version);

		g_dbus_emit_signal(connection, adapter->path,
				ADAPTER_INTERFACE,
				"DeviceIdentification",
				DBUS_TYPE_UINT16, &result,
				DBUS_TYPE_UINT16, &vendor,
				DBUS_TYPE_UINT16, &product,
				DBUS_TYPE_UINT16, &version,
				DBUS_TYPE_INVALID);
		EVENT_DRC_BLUEZ_DID(FORMAT_ID_DID, SIGNAL_DEVICEIDENTIFICATION, result, vendor, product, version);
	}
