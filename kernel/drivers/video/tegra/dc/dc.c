		       ...
		       void (* print)(void *data, const char *str))
{
	...
	DUMP_REG(DC_COM_PIN_OUTPUT_POLARITY1);
	DUMP_REG(DC_COM_PIN_OUTPUT_POLARITY2);
	DUMP_REG(DC_COM_PIN_OUTPUT_POLARITY3);
...
static int tegra_dc_update_winlut(struct tegra_dc *dc, int win_idx, int fbovr)
{
	...
	tegra_dc_update_windows(&win, 1, true);
			    ...
			    bool wait_for_vblank)
{
	...
	for (i = 0; i < n; i++) {
		...
		struct tegra_dc_win *dc_win =
			tegra_dc_get_window(dc, win->idx);
		unsigned h_dda;
		unsigned v_dda;
	...
	if (wait_for_vblank) {
		...
		/* Don't use interrupt handler for this update.
		   Poll HW until window update is completed. This may block
		   for as long as it takes to display one scanline. */
		while (tegra_dc_windows_are_dirty(dc))
			udelay(1);
...
int tegra_dc_sync_windows(struct tegra_dc_win *windows[], int n)
{
	...
	ret = wait_event_interruptible_timeout(windows[0]->dc->wq,
					 tegra_dc_windows_are_clean(windows, n),
/* FUJITSU TEN:2012-10-23 Generic Porting modified. start */
#if 0
					 HZ);
#endif
					 HZ/20);
/* FUJITSU TEN:2012-10-23 Generic Porting modified. end */
...
static void tegra_dc_underflow_handler(struct tegra_dc *dc)
{
	...
	dc->underflow_mask = 0;
	spin_lock(&dc->irq_lock);
	val = tegra_dc_readl(dc, DC_CMD_INT_MASK);
	tegra_dc_writel(dc, val | ALL_UF_INT, DC_CMD_INT_MASK);
	spin_unlock(&dc->irq_lock);
	print_underflow_info(dc);
}

#ifndef CONFIG_TEGRA_FPGA_PLATFORM
bool tegra_dc_windows_are_dirty(struct tegra_dc *dc)
{
...
static void tegra_dc_trigger_windows(struct tegra_dc *dc)
{
	...
	if (!dirty) {
		...
		if (dc->out->flags & TEGRA_DC_OUT_ONE_SHOT_MODE)
			val &= ~V_BLANK_INT;
		else if (!atomic_read(&dc->frame_end_ref))
			val &= ~FRAME_END_INT;
		tegra_dc_writel(dc, val, DC_CMD_INT_MASK);
		spin_unlock(&dc->irq_lock);
	}
...
int tegra_dc_config_frame_end_intr(struct tegra_dc *dc, bool enable)
{
	...
	spin_lock_irqsave(&dc->irq_lock, flags);
	if (enable) {
	...
	spin_unlock_irqrestore(&dc->irq_lock, flags);
	...
	struct nvhost_device_id *id_table)
{
	...
	spin_lock_init(&dc->irq_lock);
	atomic_set(&dc->frame_end_ref, 0);
	mutex_init(&dc->lock);
...
{
	struct tegra_dc *dc = nvhost_get_drvdata(ndev);
	/* FUJITSU TEN:2013-07-30 Color Adjust ADD start */
	int	cnt;
	/* FUJITSU TEN:2013-07-30 Color Adjust ADD end */
	...
	for (cnt = 0; cnt < dc->n_windows; cnt++) {
		struct tegra_dc_win *win = &dc->windows[cnt];
		win->idx = cnt;
		win->dc = dc;
		tegra_dc_init_csc_defaults(&win->csc);
		tegra_dc_init_lut_defaults(&win->lut);
		tegra_dc_writel(dc, WINDOW_A_SELECT << cnt,
				DC_CMD_DISPLAY_WINDOW_HEADER);
		tegra_dc_set_csc(dc, &win->csc);
		tegra_dc_set_lut(dc, win);
	}
...
#ifndef MODULE
int tegra_dc_parse_mode_params(const char *options, struct tegra_dc_mode *mode)
{
	if (sscanf(options, "%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
				&mode->pclk,
				&mode->h_active,
				&mode->v_active,
				&mode->h_ref_to_sync,
...
static int __init tegra_dc_mode_override(char *str)
{
	...
	if (p) {
		p += 5;
		options = strsep(&p, ";");
		if (tegra_dc_parse_mode_params(options,
