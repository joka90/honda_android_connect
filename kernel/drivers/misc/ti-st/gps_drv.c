...
/* FUJITSU TEN :2012-11-15 Support_BuDet start */
#ifdef CONFIG_TI_FASTBOOT
	long timeout = GPSDRV_REG_TIMEOUT;
...
/* FUJITSU TEN :2012-11-15 Support_BuDet start */
#ifdef CONFIG_TI_FASTBOOT
		timeout = wait_for_completion_budet_interruptible_timeout \
		(&hgps->gpsdrv_reg_completed, msecs_to_jiffies(timeout));
		/* Check for budet interrupt condition */
		if (-ERESTARTSYS == timeout) {
			ADA_LOG_ST_TRACE("Budet Interrput occurred. err: %ld", timeout);
			DRC_ST_EVT(EVT_BUDET+31, timeout);
			clear_bit(GPS_ST_REGISTERED, &hgps->state);
			return -EINTR;
		}
#else /* CONFIG_TI_FASTBOOT */
...
/* FUJITSU TEN :2012-11-15 Support_BuDet start */
#ifdef CONFIG_TI_FASTBOOT
	long timeout = GPSDRV_READ_TIMEOUT;
#else /* CONFIG_TI_FASTBOOT */
...
/* FUJITSU TEN :2012-11-15 Support_BuDet start */
	...
	if ((-ERESTARTSYS == timeout) &&
		(get_budet_flag() )) {
		ADA_LOG_ST_TRACE("Budet Interrput occurred. err :%ld",timeout);
		DRC_ST_EVT(EVT_BUDET+32, timeout);
		return -EINTR;
	}
#endif /* CONFIG_TI_FASTBOOT */
/* FUJITSU TEN :2012-11-15 Support_BuDet end */
