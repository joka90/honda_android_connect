...
int set_pinmux(const char *pingrp, const char *setting)
{
	int i, err;

	if (!pingrp || !*pingrp || !setting || !*setting)
		return -EINVAL;

	for (i = 0; i < TEGRA_MAX_PINGROUP; i++) {
		if (strcasecmp(pingrp, pingroups[i].name) == 0) {
			if (strcasecmp(setting, "NORMAL") == 0)
				err = tegra_pinmux_set_tristate(i,
					TEGRA_TRI_NORMAL);
			else if (strcasecmp(setting, "TRISTATE") == 0)
				err = tegra_pinmux_set_tristate(i,
					TEGRA_TRI_TRISTATE);
			else {
				pr_err("%s(): Invalid pingroup state, "
					"valid are: tristate or normal\n",
					__func__);
				err = -EINVAL;
			}
			return err;
	...
	pr_err("%s(): Invalid pingroup specified\n", __func__);
	return -EINVAL;
}

ssize_t set_pinmux_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *pos)
{
	char *buf, *s, *pingrp = NULL, *setting = NULL;
	size_t wrote = count;
	int err;
