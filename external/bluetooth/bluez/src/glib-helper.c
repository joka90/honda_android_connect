...
static GSList *context_list2 = NULL;
...
static void search_context_cleanup2(struct search_context *ctxt)
{
	context_list2 = g_slist_remove(context_list2, ctxt);
...
static gboolean search_process_cb2(GIOChannel *chan, GIOCondition cond,
							gpointer user_data)
{
	struct search_context *ctxt = user_data;
	int err = 0;
...
failed:
	if (err) {
		...
		search_context_cleanup2(ctxt);
	}

	return FALSE;
}

static void search_completed_cb2(uint8_t type, uint16_t status,
			uint8_t *rsp, size_t size, void *user_data)
{
	struct search_context *ctxt = user_data;
	sdp_list_t *recs = NULL;
...
static gboolean connect_watch2(GIOChannel *chan, GIOCondition cond,
							gpointer user_data)
{
	struct search_context *ctxt = user_data;
	sdp_list_t *search, *attrids;
	...
	if (sdp_set_notify(ctxt->session, search_completed_cb2, ctxt) < 0) {
		err = EIO;
		goto failed;
	}
...
static int create_search_context2(struct search_context **ctxt,
					const bdaddr_t *src,
					const bdaddr_t *dst,
					uuid_t *uuid)
{
...
int bt_search_service2(const bdaddr_t *src, const bdaddr_t *dst,
			uuid_t *uuid, bt_callback_t cb, void *user_data,
			bt_destroy_t destroy)
{
	struct search_context *ctxt = NULL;
	...
	err = create_search_context2(&ctxt, src, dst, uuid);
	if (err < 0)
		return err;
	...
	context_list2 = g_slist_append(context_list2, ctxt);

	return 0;
}

static gint find_by_uuid(gconstpointer data, gconstpointer user_data)
{
	const struct search_context *ctxt = data, *search = user_data;

	return (bacmp(&ctxt->dst, &search->dst) &&
					bacmp(&ctxt->src, &search->src) &&
					memcmp(&ctxt->uuid, &search->uuid, sizeof(uuid_t)));
}


int bt_cancel_discovery2(const bdaddr_t *src, const bdaddr_t *dst, uint16_t svclass)
{
	struct search_context match, *ctxt;
	GSList *l;
	uuid_t uuid;
	...
	bacpy(&match.src, src);
	bacpy(&match.dst, dst);
	memcpy( &match.uuid, &uuid, sizeof(uuid_t) );

	/* Ongoing SDP Discovery */
	l = g_slist_find_custom(context_list2, &match, find_by_uuid);
	if (l == NULL)
		return -ENOENT;
