			    ...
			    void *to, unsigned long from, ssize_t len)
{
	...
	if (len > 32) {
		...
		snor_tegra_writel(c, dma_config, TEGRA_SNOR_DMA_CFG_REG);
...
static void tegra_snor_reset_controller(struct tegra_nor_info *info)
{
	...
	snor_config = snor_tegra_readl(info, TEGRA_SNOR_CONFIG_REG);
	dma_config = snor_tegra_readl(info, TEGRA_SNOR_DMA_CFG_REG);
	timing0 = snor_tegra_readl(info, TEGRA_SNOR_TIMING0_REG);
	timing1 = snor_tegra_readl(info, TEGRA_SNOR_TIMING1_REG);
	...
	snor_tegra_writel(info, snor_config, TEGRA_SNOR_CONFIG_REG);
	snor_tegra_writel(info, dma_config, TEGRA_SNOR_DMA_CFG_REG);
	snor_tegra_writel(info, timing0, TEGRA_SNOR_TIMING0_REG);
	snor_tegra_writel(info, timing1, TEGRA_SNOR_TIMING1_REG);
...
static int tegra_nor_probe(struct platform_device *pdev)
{
	...
	if (gpio_request(plat->gmi_rst_n_gpio, "GMI_RST_N"))
		return -EIO;
	gpio_direction_output(plat->gmi_rst_n_gpio, HIGH);
