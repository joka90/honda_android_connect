...
#define DBUS_DEBUG_ERROR(fmt, ...) \
		((void)ADA_LOG_PRINT_FORMAT(ANDROID_LOG_ERROR, "dbus", __FUNCTION__, fmt, ##__VA_ARGS__))
#define BLUEZ_DEBUG_LOG(fmt, ...)   \
		((void)ADA_LOG_PRINT_FORMAT(ANDROID_LOG_DEBUG, "dbus", __FUNCTION__, fmt, ##__VA_ARGS__))
...
main (int argc, char **argv)
{
  ...
  BLUEZ_DEBUG_LOG("setOomAdjSystem Start.");
  if(OOM_OK != setOomAdjSystem()){
    DBUS_DEBUG_ERROR("setOomAdjSystem Fail.");
  }
