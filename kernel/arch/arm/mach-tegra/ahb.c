...
#ifdef CONFIG_ARCH_TEGRA_2x_SOC
	val = gizmo_readl(AHB_GIZMO_SDMMC1);
	val |= ENABLE_SPLIT;
	gizmo_writel(val, AHB_GIZMO_SDMMC1);

	val = gizmo_readl(AHB_GIZMO_SDMMC2);
	val |= ENABLE_SPLIT;
	gizmo_writel(val, AHB_GIZMO_SDMMC2);

	val = gizmo_readl(AHB_GIZMO_SDMMC3);
	val |= ENABLE_SPLIT;
	gizmo_writel(val, AHB_GIZMO_SDMMC3);

	val = gizmo_readl(AHB_GIZMO_SDMMC4);
	val |= ENABLE_SPLIT;
	gizmo_writel(val, AHB_GIZMO_SDMMC4);

	val = gizmo_readl(AHB_GIZMO_NOR);
	val |= ENABLE_SPLIT;
	gizmo_writel(val, AHB_GIZMO_NOR);
#endif
