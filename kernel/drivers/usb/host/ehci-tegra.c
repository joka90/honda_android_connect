...
#ifdef CONFIG_ARCH_TEGRA_2x_SOC
	/* Set the TXFILLTUNING register and AHB-Prefetch registers
	   to avoid TX FIFO underrun and Ethernet lockup in Tegra 2 */

	dev_notice(&pdev->dev, "setting TXFILLTUNING registers\n");
	temp = readl(hcd->regs + USB_TXFILLTUNING);
	if ((temp & 0x00FF0000) != 0x00100000) {
		temp = 0x00100000;
		writel(temp, hcd->regs + USB_TXFILLTUNING) ;
	}
	if (pdev->id == 2) {
		dev_notice(&pdev->dev, "setting AHB-prefetch registers for USB3\n");
		ahb_prefetch_reg = (IO_ADDRESS(0x6000C0F4));
		*ahb_prefetch_reg = 0xC5800800;
	}
#endif
	return err;
