...
bdaddr_t *bd_addr = NULL;

/* File descriptor for the UART device*/
int dev_fd = -1;

/* FUJITSU TEN :2013-02-21 DriveRec start */
#define BLUETOOTH_DRIVER    35
#define UIM_ERR_LEVEL       3
#define UIM_ERR_ID          2		// UIM
#define UIM_ERR_PARAM1      3		// read_hci_event
...
static void drc_uim_err(int err)
{
	int fd;
	int result;
	lites_err errinfo;
	ssize_t size = 0;
	int level = UIM_ERR_LEVEL;
	struct lites_trace_header trc_header;
	struct lites_trace_format trc_format;
	unsigned int gid = REGION_TRACE_ERROR;
	unsigned int tid = LITES_KNL_LAYER + BLUETOOTH_DRIVER;
	unsigned int tno = LITES_KNL_LAYER + BLUETOOTH_DRIVER;

	errinfo.err_id = UIM_ERR_ID;
	errinfo.param1 = UIM_ERR_PARAM1;
	errinfo.param2 = err;
	errinfo.param3 = 0;
	errinfo.param4 = 0;
	...
	fd = open("/dev/lites", O_RDWR);
	if (fd < 0) {
		UIM_ERR("open fault /dev/lites");
	} else {
		result = lites_trace_write(fd, gid, &trc_format);
		close(fd);
	}
	return;
}
...
/* FUJITSU TEN :2013-02-21 DriveRec start */
	...
	result = read_hci_event(fd, (unsigned char *)&resp, sizeof(resp));
	if (result < 0) {
		drc_uim_err(result);
/* FUJITSU TEN :2013-02-21 DriveRec end */
...
/* FUJITSU TEN :2013-05-16 klocwork 198172 start */
		len = read(fd, buf, 31);
		//len = read(fd, buf, 32);
/* FUJITSU TEN :2013-05-16 klocwork 198172 end */
		if (len < 0) {
...
/* FUJITSU TEN :2013-05-16 klocwork 198181 start */
			//close(fd);
/* FUJITSU TEN :2013-05-16 klocwork 198181 end */
			return -1;
		}
/* FUJITSU TEN :2013-05-16 klocwork 198172 start */
		len = read(fd, buf, 31);
...
/* FUJITSU TEN :2013-05-16 klocwork 198172 end */
		...
		if (dev_fd < 0) {
			...
			dev_fd = open((const char*) uart_dev_name, O_RDWR);
			if (dev_fd < 0) {
				UIM_ERR(" Can't open %s errno=%s(%d)", uart_dev_name, strerror(errno), errno);
				return -1;
			}
		}
		/*
