...
static const u32 early_emc_restore_reg[] = {
	DEFINE_REG(TEGRA_MC_BASE, MC_LATENCY_ALLOWANCE_G2_0),
	DEFINE_REG(TEGRA_MC_BASE, MC_LATENCY_ALLOWANCE_G2_1),
	DEFINE_REG(TEGRA_EMC_BASE, EMC_AUTO_CAL_CONFIG),
	DEFINE_REG(TEGRA_EMC_BASE, EMC_REFRESH),
	DEFINE_REG(TEGRA_EMC_BASE, EMC_MRS_WAIT_CNT),
	DEFINE_REG(TEGRA_EMC_BASE, EMC_AUTO_CAL_INTERVAL),
	DEFINE_REG(TEGRA_EMC_BASE, EMC_ZCAL_MRW_CMD),
...
#define EARLY_EMC_RESTORE_REG_NUM ARRAY_SIZE(early_emc_restore_reg)

static u32 early_emc_restore_val[EARLY_EMC_RESTORE_REG_NUM];
...
inline void tegra_early_emc_timing_restore(void)
{
	int i;

	for (i = 0; i < EARLY_EMC_RESTORE_REG_NUM; i++)
		__raw_writel(early_emc_restore_val[i], early_emc_restore_reg[i]);

	wmb();
	__raw_readl(early_emc_restore_reg[0]);

	emc_timing_update();
}

inline void tegra_early_emc_timing_save(void)
{
	int i;
	for (i = 0; i < EARLY_EMC_RESTORE_REG_NUM; i++)
		early_emc_restore_val[i] = __raw_readl(early_emc_restore_reg[i]);
}
