...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_PLUGINS_HCIOPS_C
								...
								uint8_t status)
{
	...
	EVENT_DRC_BLUEZ_PAIRING(FORMAT_ID_BT_PAIRING, BT_BONDING_COMP, status ,&conn->bdaddr);
	BLUEZ_DEBUG_WARN("[HCI] BDADDR = [ %02x:%02x:%02x:%02x:%02x:%02x ] status = 0x%02x ", conn->bdaddr.b[5], conn->bdaddr.b[4], conn->bdaddr.b[3], conn->bdaddr.b[2], conn->bdaddr.b[1], conn->bdaddr.b[0], status);
...
static void user_confirm_request(int index, void *ptr)
{
	...
	if (conn == NULL) {
		BLUEZ_DEBUG_WARN("[HCI] conn is NULL");
		return;
	}

	num = count_link_key( &dev->bdaddr );
	ba2str(&req->bdaddr, addr);
	BLUEZ_DEBUG_WARN("[HCI] Linkkey Num = %d BDADDR = [ %s ]", num, addr);
	EVENT_DRC_BLUEZ_PAIRING(FORMAT_ID_BT_PAIRING, BT_CONFIRM_REQ, num, &req->bdaddr);
	if( num >= MAX_PAIRING_NUM ) {
		BLUEZ_DEBUG_WARN("Rejecting request: MAX PAIRING OVER NUM");
		goto fail;
	}
...
static void pin_code_request(int index, bdaddr_t *dba)
{
	...
	num = count_link_key( &dev->bdaddr );
	BLUEZ_DEBUG_WARN("[HCI] Linkkey Num = %d BDADDR = [ %s ]",  num, addr);
	EVENT_DRC_BLUEZ_PAIRING(FORMAT_ID_BT_PAIRING, BT_PIN_REQ, num, dba);
	if( num >= MAX_PAIRING_NUM ) {
		BLUEZ_DEBUG_WARN("Rejecting PIN request: MAX PAIRING OVER NUM");
		goto reject;
	}
...
static inline void cmd_status(int index, void *ptr)
{
	...
	if (opcode == cmd_opcode_pack(OGF_LINK_CTL, OCF_CREATE_CONN) && evt->status == 0) {
		BLUEZ_DEBUG_LOG("[HCI] cmd_status() opcode = 0x%04x", opcode);
		btd_manager_receive_cmd_status( evt->status, evt->ncmd, evt->opcode );
	}
	return;
}
...
static inline void conn_complete(int index, void *ptr)
{
	...
	BLUEZ_DEBUG_INFO("[HCI] conn_complete() link_type = %d handle = 0x%04x status = 0x%02x", evt->link_type, btohs(evt->handle), evt->status);

	if (evt->link_type != ACL_LINK) {
		btd_manager_receive_conn_complete( evt->status, btohs(evt->handle), &evt->bdaddr, evt->link_type, evt->encr_mode );
		return;
	}
	...
	if (evt->status != 0) {
		reject = btd_manager_find_reject_info( &evt->bdaddr );
		if( reject == FALSE ) {
			conn_failed(index, &evt->bdaddr, evt->status);
			btd_manager_receive_conn_complete( evt->status, btohs(evt->handle), &evt->bdaddr, evt->link_type, evt->encr_mode );
		} else {
			BLUEZ_DEBUG_WARN("[HCI] Connection complete ignore");
			EVENT_DRC_BLUEZ_MANAGER(FORMAT_ID_BT_MANAGER, MANAGER_COMP_IGNORE, &evt->bdaddr, evt->status);
			/* ACL Šm—§ƒK[ƒh‰ðœ */
...
static inline void disconn_complete(int index, void *ptr)
{
	...
	BLUEZ_DEBUG_INFO("[HCI] disconn_complete() handle = 0x%04x status = 0x%02x reason = 0x%02x", btohs(evt->handle), evt->status, evt->reason);
	DBG("handle %u status 0x%02x", btohs(evt->handle), evt->status);

	if (evt->status != 0)
		return;

	btd_manager_receive_disconn_complete( evt->status, btohs(evt->handle), evt->reason );
...
static inline void conn_request(int index, void *ptr)
{
	...
	BLUEZ_DEBUG_INFO("[HCI] conn_request() link_type = %u class = 0x%04x", evt->link_type, class);
	BLUEZ_DEBUG_WARN("[HCI] link_type = %u class = 0x%04x [ %02x:%02x:%02x:%02x:%02x:%02x ]", evt->link_type, class, evt->bdaddr.b[5], evt->bdaddr.b[4], evt->bdaddr.b[3], evt->bdaddr.b[2], evt->bdaddr.b[1], evt->bdaddr.b[0]);

	if( evt->link_type ==  ACL_LINK ) {
		/* Accept / Reject ”»’è */
		if( TRUE == btd_manager_check_acl( &evt->bdaddr ) ) {
			/* Reject */
			/* CONNECTION REJECTED DUE TO LIMITED RESOURCES (0X0D) */
			bluezutil_reject_conn_req( &evt->bdaddr, HCI_REJECTED_LIMITED_RESOURCES );
			btd_manager_set_reject_info( &evt->bdaddr );
	...
	btd_manager_receive_conn_request( &evt->bdaddr, evt->dev_class, evt->link_type );
}
...
static inline void sync_conn_complete(int index, void *ptr)
{
	struct dev_info *dev = &devs[index];
	evt_sync_conn_complete *evt = ptr;

	BLUEZ_DEBUG_INFO("[HCI] sync_conn_complete() handle = %u link_type = %u", btohs(evt->handle), evt->link_type);

	btd_manager_receive_sync_conn_complete( evt->status, 
											evt->handle, 
											&evt->bdaddr, 
											evt->link_type, 
											evt->trans_interval, 
...
static inline void sync_conn_changed(int index, void *ptr)
{
	struct dev_info *dev = &devs[index];
	evt_sync_conn_changed *evt = ptr;

	BLUEZ_DEBUG_INFO("[HCI] sync_conn_changed() handle = %u", btohs(evt->handle));

	btd_manager_receive_sync_conn_changed( evt->status, 
										   evt->handle, 
										   evt->trans_interval, 
										   evt->retrans_window, 
										   evt->rx_pkt_len, 
...
static inline void hci_evt_rcv(int index, void *ptr, int len)
{
	struct dev_info *dev = &devs[index];
	receive_packet *evt = ptr;

	BLUEZ_DEBUG_INFO("[HCI] hci_evt_rcv()");
	
	btd_manager_receive_hci_event_packet( (void *)evt, len );
}
								...
								gpointer data)
{
	...
	if( gBsmMode ) {
		...
		for(i=0; i<len; i++)
		{
			BLUEZ_DEBUG_LOG("ptr[%02d] = 0x%02x",i, *(ptr + i) );
		}
		hci_evt_rcv(index, ptr, len);
	}
	else {
		type = *ptr++;
		...
		case EVT_SYNC_CONN_COMPLETE:
			sync_conn_complete(index, ptr);
			break;

		case EVT_SYNC_CONN_CHANGED:
			sync_conn_changed(index, ptr);
			break;
		}
	}
...
static void start_hci_dev(int index)
{
	...
	if (setsockopt(dev->sk, SOL_HCI, HCI_FILTER, &flt, sizeof(flt)) < 0) {
		...
		BLUEZ_DEBUG_ERROR("[Bluez - HCI] Can't set filter on hci%d: %s (%d)",
						index, strerror(errno), errno);
		ERR_DRC_BLUEZ(ERROR_BLUEZ_HCI_FILTER, 0, 0);
...
static int hciops_start_discovery(int index)
{
	...
	EVENT_DRC_BLUEZ_DISCOVERY(FORMAT_ID_BT_DISCOVERY, BT_START_DISCOVERY, adapter_type);
...
static int hciops_stop_discovery(int index)
{
	...
	EVENT_DRC_BLUEZ_DISCOVERY(FORMAT_ID_BT_DISCOVERY, BT_STOP_DISCOVERY, dev->discov_state);
...
static int hciops_create_bonding(int index, bdaddr_t *bdaddr, uint8_t io_cap)
{
	...
	EVENT_DRC_BLUEZ_PAIRING(FORMAT_ID_BT_PAIRING, BT_START_PAIRING, io_cap, bdaddr);
	BLUEZ_DEBUG_WARN("[HCI] create_bonding [ %02x:%02x:%02x:%02x:%02x:%02x ]", bdaddr->b[5], bdaddr->b[4], bdaddr->b[3], bdaddr->b[2], bdaddr->b[1], bdaddr->b[0]);

	if( TRUE == btd_manager_check_acl( bdaddr ) ) {
		ERR_DRC_BLUEZ(ERROR_BLUEZ_PAIRING, 0, 0);
		return -EBUSY;
	}
	...
	if (conn->io == NULL) {
		...
		BLUEZ_DEBUG_ERROR("[Bluez - HCI] bt_io_connect() ERROR : %s", err->message);
		ERR_DRC_BLUEZ(ERROR_BLUEZ_PAIRING, err->code, 0);
		g_error_free(err);
		return -EIO;
	}
...
static int hciops_cancel_bonding(int index, bdaddr_t *bdaddr)
{
	...
	EVENT_DRC_BLUEZ_PAIRING(FORMAT_ID_BT_PAIRING, BT_CANCEL_PAIRING, 0, bdaddr);
	BLUEZ_DEBUG_WARN("[HCI] cancel_bonding [ %02x:%02x:%02x:%02x:%02x:%02x ]", bdaddr->b[5], bdaddr->b[4], bdaddr->b[3], bdaddr->b[2], bdaddr->b[1], bdaddr->b[0]);

	conn = find_connection(dev, bdaddr);
	if (conn == NULL || conn->io == NULL){
		BLUEZ_DEBUG_ERROR("[Bluez - HCI] find_connection() ERROR");
		ERR_DRC_BLUEZ(ERROR_BLUEZ_PAIRING, 0, 0);
		return -ENOTCONN;
	}
...
static int hciops_set_link_timeout(int index, bdaddr_t *bdaddr, uint32_t num_slots)
{
	...
	ba2str(bdaddr, addr);
	DBG("hci%d, addr %s, num_slots %d", index, addr, num_slots);
...
int HCI_get_device_id()
{
	BLUEZ_DEBUG_LOG("[HCI][HCI_get_device_id]  max_dev=%d ",  max_dev);
	if(max_dev < 0){
		return 0;
	}
	BLUEZ_DEBUG_LOG("[HCI][HCI_get_device_id] devs[%d].id=%d", max_dev, devs[max_dev].id);
	return devs[max_dev].id;
}
