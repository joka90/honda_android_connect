...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_LIB_HCI_C
...
int hci_cmd_send_req(int dd, struct hci_request *r, int to)
{
	unsigned char buf[HCI_MAX_EVENT_SIZE], *ptr;
	uint16_t opcode = htobs(cmd_opcode_pack(r->ogf, r->ocf));
	struct hci_filter nf, of;
...
int hci_read_local_name2(int dd, uint8_t *status, int len, char *name, int to)
{
	read_local_name_rp rp;
	struct hci_request rq;
	*status = 0x90;
...
int hci_write_local_name2(int dd, uint8_t *status, const char *name, int to)
{
	change_local_name_cp cp;
	struct hci_request rq;
...
int hci_read_local_version2(int dd, uint8_t *status, struct hci_version *ver, int to)
{
	read_local_version_rp rp;
	struct hci_request rq;
...
int hci_read_bd_addr2(int dd, uint8_t *status, bdaddr_t *bdaddr, int to)
{
	read_bd_addr_rp rp;
	struct hci_request rq;
...
int hci_write_class_of_dev2(int dd, uint8_t *status, uint32_t cls, int to)
{
	write_class_of_dev_cp cp;
	struct hci_request rq;
...
int hci_write_simple_pairing_mode2(int dd, uint8_t *status, uint8_t mode, int to)
{
	write_simple_pairing_mode_cp cp;
	write_simple_pairing_mode_rp rp;
	struct hci_request rq;
...
int hci_write_event_mask(int dd, uint8_t *mask, int to)
{
	set_event_mask_cp cp;
	uint8_t status;
	struct hci_request rq;

	memset(&cp, 0, sizeof(cp));
	memcpy(mask, cp.mask, 8);
...
int hci_write_event_mask2(int dd, uint8_t *hci_sts, uint8_t *mask, int to)
{
	set_event_mask_cp cp;
	uint8_t status;
	struct hci_request rq;
...
int hci_vs_write_codec_config(int dd, uint8_t *status, uint8_t mode, int to)
{
	...
	*status = 0x90;
	...
	if( mode )
	{
		cp.clock_rate = 0x0400;
		cp.freq       = 0x00003E80;
	}
	else
	{
		cp.clock_rate = 0x0200;
		cp.freq       = 0x00001F40;
	}
	cp.clock_direction     = 0x01;
	cp.cycle               = 0x0020;
	cp.edge                = 0x00;
	cp.polarity            = 0x01;
	cp.reserved1           = 0x00;
	cp.ch1_data_out_size   = 0x0010;
	cp.ch1_data_out_offset = 0x0001;
	cp.ch1_data_out_edge   = 0x01;
	cp.ch1_data_in_size    = 0x0010;
	cp.ch1_data_in_offset  = 0x0001;
	cp.ch1_data_in_edge    = 0x00;
	cp.reserved2           = 0x00;
	cp.ch2_data_out_size   = 0x0010;
	cp.ch2_data_out_offset = 0x0021;
	cp.ch2_data_out_edge   = 0x01;
	cp.ch2_data_in_size    = 0x0010;
	cp.ch2_data_in_offset  = 0x0021;
	cp.ch2_data_in_edge    = 0x00;
	cp.reserved3           = 0x00;
	...
	rq.cparam = &cp;
	rq.clen   = VS_WRITE_CODEC_CONFIG_CP_SIZE;
	rq.rparam = &rp;
	rq.rlen   = VS_WRITE_CODEC_CONFIG_RP_SIZE;
...
int hci_accept_conn_req(int dd, const bdaddr_t *bdaddr, uint8_t role, int to)
{
	evt_conn_complete rp;
	...
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = OGF_LINK_CTL;
	rq.ocf    = OCF_ACCEPT_CONN_REQ;
	rq.event  = EVT_CONN_COMPLETE;
	rq.cparam = &cp;
	rq.clen   = ACCEPT_CONN_REQ_CP_SIZE;
	rq.rparam = &rp;
	rq.rlen   = EVT_CONN_COMPLETE_SIZE;
...
int hci_reject_conn_req(int dd, const bdaddr_t *bdaddr, uint8_t reason, int to)
{
	evt_conn_complete rp;
	...
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = OGF_LINK_CTL;
	rq.ocf    = OCF_REJECT_CONN_REQ;
	rq.event  = EVT_CONN_COMPLETE;
	rq.cparam = &cp;
	rq.clen   = REJECT_CONN_REQ_CP_SIZE;
	rq.rparam = &rp;
	rq.rlen   = EVT_CONN_COMPLETE_SIZE;
...
int hci_send_packet(int dd, uint8_t *status, char *data, uint32_t len, int to)
{
	send_packet cp;
	receive_packet rp;
	struct hci_request rq;
	...
	if(data[0] == HCI_ACLDATA_PKT) {
		...
		memset(&rq, 0, sizeof(rq));
		memcpy(cp.packet, data+4, len - 4);
		
		opcode     = data[1];
		opcode    |= (data[2] << 8);
		rq.ogf    = opcode >> 10;
		rq.ocf    = opcode & 0x03ff;
		rq.cparam = &cp;
		rq.clen   = len - 4;
		rq.rparam = &rp;
		rq.rlen   = sizeof(rp);

		if (hci_cmd_send_req(dd, &rq, to) < 0)
		{
			return -1;
		}
	}
...
int hci_vs_wbs_disassociate(int dd, uint8_t *status, int to)
{
	vs_wbs_disassociate_rp rp;
	struct hci_request rq;
	...
	*status = 0x90;
	...
	memset(&rq, 0, sizeof(rq));
	rq.ogf    = OGF_VENDOR_CMD;
	rq.ocf    = OCF_VS_WBS_DISASSOCIATE;
	rq.rparam = &rp;
	rq.rlen   = VS_WBS_DISASSOCIATE_RP_SIZE;
