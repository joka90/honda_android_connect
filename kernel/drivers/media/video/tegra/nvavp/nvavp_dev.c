...
static void nvavp_set_channel_control_area(struct nvavp_info *nvavp, int channel_id)
{
	...
	writel(0x0, &control->idle_notify_enable);
...
static void clock_disable_handler(struct work_struct *work)
{
	...
	nvavp_clk_ctrl(nvavp, !nvavp_check_idle(nvavp, NVAVP_VIDEO_CHANNEL));
...
int nvavp_load_os(struct nvavp_info *nvavp, char *fw_os_file)
{
	struct nvavp_os_info *os_info = &nvavp->os_info;
	const struct firmware *nvavp_os_fw;
	void *ptr;
	u32 size, reloc_addr, idx;
	int ret = 0;
	...
	if (!os_info->os_bin) {
		...
		ptr += sizeof(u32);
		os_info->reloc_count = *((u32 *)ptr);
		ptr += sizeof(u32);
		os_info->code_size = *((u32 *)ptr);
		ptr += sizeof(u32);
		os_info->init_size = *((u32 *)ptr);
		ptr += sizeof(u32);

		size = os_info->init_size + (os_info->reloc_count * 4);
		os_info->size = size;

		os_info->os_bin = kzalloc(size, GFP_KERNEL);
		if (!os_info->os_bin) {
			dev_err(&nvavp->nvhost_dev->dev,
		...
		memset(os_info->data + os_info->code_size, 0,
				SZ_1M - os_info->code_size);

		dev_info(&nvavp->nvhost_dev->dev,
			"entry=%08x control=%08x debug=%08x\n",
			os_info->entry_offset, os_info->control_offset,
			os_info->debug_offset);
		dev_info(&nvavp->nvhost_dev->dev,
			"relocs=%d init_size=%d code_size=%d\n",
			os_info->reloc_count, os_info->init_size,
			os_info->code_size);
		release_firmware(nvavp_os_fw);
	}

	memcpy(os_info->data, os_info->os_bin, os_info->init_size);
	memset(os_info->data + os_info->init_size, 0,
			(os_info->code_size - os_info->init_size));

	if (os_info->reloc_count) {
		u32* reloc = (u32 *)((u32)os_info->os_bin + os_info->init_size);
		u32 value;
		for (idx = 0; idx < os_info->reloc_count; idx++) {
			reloc_addr = NVAVP_OS_GET_ADDRESS(reloc[idx]);
			value = readl((u32)os_info->data + reloc_addr);
			value += os_info->phys;
			writel(value, (u32)os_info->data + reloc_addr);
		}
	}
...
static int tegra_nvavp_suspend(struct nvhost_device *ndev, pm_message_t state)
{
	...
	if (nvavp->refcount) {
		if (nvavp_check_idle(nvavp, NVAVP_VIDEO_CHANNEL) ) {
#if defined(CONFIG_TEGRA_NVAVP_AUDIO)
