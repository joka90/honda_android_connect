...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_BTIO_BTIO_C
...
typedef struct drc_bluez_btio_error_log  {
...
} DRC_BLUEZ_BTIO_ERROR_LOG;

static int _ERR_DRC_BLUEZ_BTIO(int errorId, int srcId, int line, int para1, int para2);

#define ERR_DRC_BLUEZ_BTIO( errorId, para1, para2 )	{	\
	_ERR_DRC_BLUEZ_BTIO( errorId, BLUEZ_SRC_ID, __LINE__, \
	para1, para2); }
							...
							gpointer user_data)
{
	...
	BLUEZ_DEBUG_LOG("*** [Bluez] connect_cb() Strat ***");
				...
				gpointer user_data, GDestroyNotify destroy)
{
    ...
    BLUEZ_DEBUG_LOG("*** [Bluez] connect_add() Strat ***");
...
static int rfcomm_connect(int sock, const bdaddr_t *dst, uint8_t channel)
{
	...
	ba2str(dst, BdAddr);
	pBdAddr = &BdAddr[0];
	BLUEZ_DEBUG_LOG("*** [Bluez] rfcomm_connect() -> connect() Start. sock=%d. bdaddr=%s. channel=%d. ***", sock, pBdAddr, channel);

	err = connect(sock, (struct sockaddr *) &addr, sizeof(addr));
	BLUEZ_DEBUG_LOG("*** [Bluez] err=%d. errno=%s(%d) ***", err, strerror(errno), errno);
	if (err < 0 && !(errno == EAGAIN || errno == EINPROGRESS))
		return err;
...
/* 14A-DA :2013-04-23 ADD start */
	addr.sco_pkt_type = SCO_LINK;
/* 14A-DA :2013-04-23 ADD end */
...
/* 14A-DA :2013-04-23 ADD start */
static int esco_connect(int sock, const bdaddr_t *dst)
{
	struct sockaddr_sco addr;
	int err;
				...
				GError **gerr, BtIOOption opt1, ...)
{
	...
	BLUEZ_DEBUG_LOG("*** [Bluez] bt_io_connect() Start. type = %d ***", type);
	...
	case BT_IO_L2CAP:
		...
		break;
	case BT_IO_RFCOMM:
		BLUEZ_DEBUG_LOG("*** [Bluez] bt_io_connect() BT_IO_RFCOMM. ***");
		char address[18];
		ba2str(&opts.dst, address);
		BLUEZ_DEBUG_LOG("*** [Bluez] bt_io_connect() address:%s. ***", address);
...
/* 14A-DA :2013-04-23 ADD start */
	case BT_IO_ESCO:
		err = esco_connect(sock, &opts.dst);
		break;
/* 14A-DA :2013-04-23 ADD end */
	default:
	...
	if (err < 0) {
		...
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] ERROR [ %s ] ( %d ). ", strerror(-err), -err);
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, err, 0);
...
static gboolean rfcomm_set2(int sock, int sec_level, int master,
				uint8_t force_active, GError **err)
{
	if (sec_level && !set_sec_level(sock, BT_IO_RFCOMM2, sec_level, err)){
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] set_sec_level() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return FALSE;
	}

	if (!set_force_active(sock, BT_IO_RFCOMM2, force_active, err)){
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] set_force_active() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return FALSE;
	}
	...
	if (master >= 0 && rfcomm_set_master(sock, master) < 0) {
		ERROR_FAILED(err, "rfcomm_set_master", errno);
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] rfcomm_set_master() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
		return FALSE;
	}
...
GIOChannel *bt_io_listen_rfcomm2(BtIOConfirm confirm, GError **err, BtIOOption opt1, ...)
{
	GIOChannel *io;
	va_list args;
	struct set_opts opts;
	...
	BLUEZ_DEBUG_LOG("*** [Bluez] bt_io_listen_rfcomm2() Start.  ***");
	...
	if (ret == FALSE) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] parse_set_opts() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, 0, 0);
		return NULL;
	}

	sock = socket(PF_BLUETOOTH, SOCK_STREAM, BTPROTO_RFCOMM2);
	if (sock < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] socket() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return NULL;
	}
	if (rfcomm_bind(sock, &opts.src, opts.channel , err) < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] rfcomm_bind() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		close(sock);
		return NULL;
	}
	if (!rfcomm_set2(sock, opts.sec_level, opts.master, opts.force_active, err)) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] rfcomm_set2() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		close(sock);
		return NULL;
	}
	...
	if (confirm) {
		BLUEZ_DEBUG_LOG("opts.defer = %d", opts.defer);
		setsockopt(sock, SOL_BLUETOOTH, BT_DEFER_SETUP, &opts.defer,
							sizeof(opts.defer));
	}

	if (listen(sock, 5) < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] listen() ERROR!!");
		ERROR_FAILED(err, "listen", errno);
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
		g_io_channel_unref(io);
		return NULL;
	...
	server_add(io, NULL, confirm, NULL, NULL);

	return io;
}

static gboolean l2cap_set2(int sock, int sec_level, uint16_t imtu,
				uint16_t omtu, uint8_t mode, int master,
				int flushable, uint8_t force_active, GError **err)
{
	if (imtu || omtu || mode) {
		...
		if (getsockopt(sock, SOL_L2CAP, L2CAP_OPTIONS, &l2o,
								&len) < 0) {
			ERROR_FAILED(err, "getsockopt(L2CAP_OPTIONS)", errno);
			BLUEZ_DEBUG_ERROR("[Bluez - BTIO] getsockopt() ERROR!!");
			ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
			return FALSE;
		}
		...
		if (setsockopt(sock, SOL_L2CAP, L2CAP_OPTIONS, &l2o,
							sizeof(l2o)) < 0) {
			ERROR_FAILED(err, "setsockopt(L2CAP_OPTIONS)", errno);
			BLUEZ_DEBUG_ERROR("[Bluez - BTIO] setsockopt() ERROR!!");
			ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
			return FALSE;
		}
	}
	...
	if (master >= 0 && l2cap_set_master(sock, master) < 0) {
		ERROR_FAILED(err, "l2cap_set_master", errno);
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] l2cap_set_master() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
		return FALSE;
	}
	...
	if (flushable >= 0 && l2cap_set_flushable(sock, flushable) < 0) {
		ERROR_FAILED(err, "l2cap_set_flushable", errno);
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] l2cap_set_flushable() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, errno);
		return FALSE;
	}

	if (sec_level && !set_sec_level(sock, BT_IO_L2CAP2, sec_level, err)){
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] set_sec_level() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return FALSE;
	}

	if (!set_force_active(sock, BT_IO_L2CAP2, force_active, err)){
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] set_force_active() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return FALSE;
	}
...
GIOChannel *bt_io_listen_l2cap2(BtIOConfirm confirm, GError **err, BtIOOption opt1, ...)
{
	GIOChannel *io;
	va_list args;
	struct set_opts opts;
	...
	BLUEZ_DEBUG_LOG("*** [Bluez] bt_io_listen_l2cap2() Start.  ***");
	...
	sock = socket(PF_BLUETOOTH, SOCK_SEQPACKET, BTPROTO_L2CAP2);
	if (sock < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] socket() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		return NULL;
	}
	if (l2cap_bind(sock, &opts.src, opts.psm, opts.cid, err) < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] l2cap_bind() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		close(sock);
		return NULL;
	}
	if (!l2cap_set(sock, opts.sec_level, opts.imtu, opts.omtu,
				opts.mode, opts.master, opts.flushable,
				opts.force_active, err)) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] l2cap_set() ERROR!!");
		ERR_DRC_BLUEZ_BTIO(ERROR_BLUEZ_BTIO, sock, 0);
		close(sock);
		return NULL;
	}
...
static int _ERR_DRC_BLUEZ_BTIO(int errorId, int srcId, int line, int para1, int para2)
{
	...
	struct drc_bluez_btio_error_log  logBuf;	    // エラーログ格納バッファ
	...
	if(fd < 0){
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] open() error. fd = %d", fd);
		return -1;
	}
    ...
    trc_fmt.trc_header->trace_no = TRACE_NO_LIB_BLUEZ;		// トレース番号(星取表のNo)
	...
	if (ret < 0) {
		BLUEZ_DEBUG_ERROR("[Bluez - BTIO] lites_trace_write() error ret = %d", ret);
	}
