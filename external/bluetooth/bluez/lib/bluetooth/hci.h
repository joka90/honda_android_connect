...
#define HCIGETROLE	_IOW('H', 250, int)
#define HCISETACCON	_IOW('H', 251, int)
...
typedef struct {
	uint16_t	clock_rate;
	uint8_t		clock_direction;
	uint32_t	freq;
	uint16_t	cycle;
	uint8_t		edge;
	uint8_t		polarity;
	uint8_t		reserved1;
	uint16_t	ch1_data_out_size;
	uint16_t	ch1_data_out_offset;
	uint8_t		ch1_data_out_edge;
	uint16_t	ch1_data_in_size;
	uint16_t	ch1_data_in_offset;
	uint8_t		ch1_data_in_edge;
	uint8_t		reserved2;
	uint16_t	ch2_data_out_size;
	uint16_t	ch2_data_out_offset;
	uint8_t		ch2_data_out_edge;
	uint16_t	ch2_data_in_size;
	uint16_t	ch2_data_in_offset;
	uint8_t		ch2_data_in_edge;
	uint8_t		reserved3;
} __attribute__ ((packed)) vs_write_codec_config_cp;
#define VS_WRITE_CODEC_CONFIG_CP_SIZE 34
typedef struct {
	uint8_t		status;
} __attribute__ ((packed)) vs_write_codec_config_rp;
#define VS_WRITE_CODEC_CONFIG_RP_SIZE 1

typedef struct {
	uint8_t		packet[HCI_MAX_FRAME_SIZE];
} __attribute__ ((packed)) send_packet;

#define OCF_VS_WBS_DISASSOCIATE	0x0179
typedef struct {
	uint8_t		status;
} __attribute__ ((packed)) vs_wbs_disassociate_rp;
#define VS_WBS_DISASSOCIATE_RP_SIZE 1
...
struct hci_role_info_req {
	bdaddr_t bdaddr;
	uint8_t  type;
	uint8_t  role;
};

struct hci_ac_conn_req {
	bdaddr_t bdaddr;
	uint8_t  dev_class[3];
	uint8_t  link_type;
};
