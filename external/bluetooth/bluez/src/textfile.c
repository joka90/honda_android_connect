...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_SRC_TEXTFILE_C

#ifndef HAVE_FDATASYNC
#define fdatasync fsync
#endif

#define LINKKEY_SIZE 55
...
int textfile_isexist(const char *filepath)
{
	int rc = 0;
	struct stat sb;
...
static int is_file_delete(char *filepath)
{
	int rc = 0;
	struct stat sb;
	...
	BLUEZ_DEBUG_LOG("[TEXT] File delete [ %s ]", filepath);
	return(1);
}

int recursive_dir_clear(char *filepath)
{
	int rc = 0;
	...
	BLUEZ_DEBUG_LOG("[TEXT] Folder delete [ %s ]", filepath);
...
static int rmdir_path(char *filepath)
{
	int rc = 0;
...
char *textfile_get_top(const char *filepath)
{
	struct stat st;
	char *map, *off, *end, *str = NULL;
	off_t size; size_t len;
...
int create_path(char *buf, size_t size, const char *path, const char *address)
{
	return snprintf(buf, size, "%s/%s/", path, address);
}

uint8_t textfile_get_linkkey_num(const char *filepath)
{
	struct stat st;
	char *map, *off, *end, *str = NULL;
	off_t size; size_t len;
