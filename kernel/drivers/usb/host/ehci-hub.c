...
/* FUJITSU TEN :2013-10-18 #53780 Support_BuDet start */
	...
	if ( get_budet_flag() ) {
		//ehci_warn(ehci, "%s(1) Budet Interrupt occurred.", __func__);
		DRC_USB_EVT(EVT_BUDET+31, 0);
	}
#endif
/* FUJITSU TEN :2013-10-18 #53780 Support_BuDet end */
...
#ifdef CONFIG_USB_FASTBOOT
	if ( get_budet_flag() ) {
		//ehci_warn(ehci, "%s(2) Budet Interrupt occurred.", __func__);
		DRC_USB_EVT(EVT_BUDET+32, 0);
	}
#endif
/* FUJITSU TEN :2013-10-18 #53780 Support_BuDet end */
...
static int ehci_bus_suspend (struct usb_hcd *hcd)
{
	...
	if (hcd->self.root_hub->do_remote_wakeup) {
		port = HCS_N_PORTS(ehci->hcs_params);
		while (port--) {
/* FUJITSU TEN :2014-01-09 iPod Denhen start */
#if 1
			if (get_budet_flag()) {
				if (ehci->reset_done[port] != 0) {
					printk(KERN_WARNING "## %s budet %d.\n",__func__, ehci->reset_done[port]);
				}
				break;
			}
#endif
/* FUJITSU TEN :2014-01-09 iPod Denhen end */
			if (ehci->reset_done[port] != 0) {
