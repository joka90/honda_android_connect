...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	long timeleft;
	int status;
	/* FUJITSU TEN:2013-10-03 #53780 USB disconnect start */
	long result;
	long onewait = HZ/10;
	/* FUJITSU TEN:2013-10-03 #53780 USB disconnect end */
	...
	/* FUJITSU TEN:2013-10-03 #53780 USB disconnect start */
#if 1
	result = timeout ? : MAX_SCHEDULE_TIMEOUT;
	timeleft = 0;
	while (result > 0) {
		if (!us->pusb_dev) {
			printk(KERN_WARNING "### %s us->pusb_dev is Null.\n",__func__);
			break;
		}
		if (!(us->pusb_dev->state == USB_STATE_UNAUTHENTICATED ||
			us->pusb_dev->state == USB_STATE_DEFAULT ||
			us->pusb_dev->state == USB_STATE_ADDRESS ||
			us->pusb_dev->state == USB_STATE_CONFIGURED)) {
			printk(KERN_WARNING "### %s canceled state=%d.\n",__func__, us->pusb_dev->state);
			break;
		}
		timeleft = wait_for_completion_interruptible_timeout(&urb_done, onewait);
		if (timeleft == 0) {		// timeout
			result -= onewait;
		} else if (timeleft > 0) {  // success
			break;
#ifdef CONFIG_USB_FASTBOOT
		} else if (timeleft == -ERESTARTSYS) {	//Budet
			//printk(KERN_WARNING "### %s Budet.\n",__func__);
			DRC_USB_EVT(EVT_BUDET+121, 0);
			break;
/* FUJITSU TEN :2014-02-20 Denhen start */
		} else if (get_budet_flag()) {
			printk(KERN_WARNING "### %s @budet.\n",__func__);
			timeleft = -ERESTARTSYS;
			break;
/* FUJITSU TEN :2014-02-20 Denhen end */
#endif
		} else {					// signal
			printk(KERN_WARNING "### %s signal=%ld.\n",__func__, timeleft);
			break;
		}
	}
	if (result <= 0) {
		printk(KERN_WARNING "### %s timedout state=%d.\n",__func__, us->pusb_dev->state);
	}
#else
...
/* FUJITSU TEN :2013-10-17 #53780 Support_BuDet start */
	...
	while (result3 > 0) {
		...
		remain = wait_event_interruptible_timeout(us->delay_wait,
				test_bit(US_FLIDX_DISCONNECTING, &us->dflags),
				onewait);
		if (remain == 0) {		// timeout
			result3 -= onewait;
		} else if (remain > 0) {  // success
			break;
 #ifdef CONFIG_USB_FASTBOOT
		} else if (remain == -ERESTARTSYS) {
			//printk(KERN_WARNING "### %s Budet.\n",__func__);
			DRC_USB_EVT(EVT_BUDET+122, 0);
			set_bit(US_FLIDX_DISCONNECTING, &us->dflags);
/* FUJITSU TEN :2014-02-20 Denhen start */
		} else if (get_budet_flag()) {
			printk(KERN_WARNING "### %s @budet.\n",__func__);
