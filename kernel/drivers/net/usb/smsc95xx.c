					 ...
					 struct sk_buff *skb, gfp_t flags)
{
	...
	/* workaround for tegra suggested by SMSC. Byte align skb->len */
	alignment = (skb->len)&3; /*same as skb->len%4 */
	if (alignment)
		alignment = 4 - alignment;

	if (skb_headroom(skb) < overhead || alignment) {
		struct sk_buff *skb2 = skb_copy_expand(skb,
			(skb_headroom(skb) < overhead) ? overhead :
			skb_headroom(skb), alignment, flags);
