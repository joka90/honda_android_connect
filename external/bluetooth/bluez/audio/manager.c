...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_AUDIO_MANAGER_C
...
static int max_connected_handsfree = 1;
static int max_connected_pbap = 1;
static int max_connected_a2dpad = 1;
static int max_connected_avrcpad = 1;
static int max_connected_browsing = 1;
static DBusConnection *connection = NULL;
static GKeyFile *config = NULL;
static GSList *adapters = NULL;
static GSList *devices = NULL;
static int pbap_rfcommCh = 0xFF;
static bdaddr_t pbap_bdaddr;
...
static void handle_uuid(const char *uuidstr, struct audio_device *device)
{
    ...
    DBG("*** audio(manager.c) handle_uuid() Start. ***");
    ...
    DBG("*** audio(manager.c) handle_uuid() uuid16 = %d. ***", uuid16);

    if(device->handsfree == NULL){
		DBG("*** handle_uuid() -> handsfree_init() Start. ***");
        device->handsfree = handsfree_init(device);
    }

    if(device->a2dpad == NULL){
		DBG("*** handle_uuid() -> a2dpad_init() Start. ***");
        device->a2dpad = a2dpad_init(device);
    }
    
    if(device->avrcpad == NULL){
		DBG("*** handle_uuid() -> avrcpad_init() Start. ***");
        device->avrcpad = avrcpad_init(device);
    }
    
     if(device->browsing == NULL){
		DBG("*** handle_uuid() -> browsing_init() Start. ***");
        device->browsing = browsing_init(device);
    }
    
     if(device->pbap == NULL){
		DBG("*** handle_uuid() -> pbap_init() Start. ***");
        device->pbap = pbap_init(device);
    }
					...
					gboolean connected)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (interface && !strcmp(AUDIO_HANDSFREE_INTERFACE, interface)
				&& !dev->handsfree)
			continue;
		if (interface && !strcmp(AUDIO_A2DPAD_INTERFACE, interface)
				&& !dev->a2dpad)
			continue;
		if (interface && !strcmp(AUDIO_AVRCPAD_INTERFACE, interface)
				&& !dev->avrcpad)
			continue;
		if (interface && !strcmp(AUDIO_BROWSING_INTERFACE, interface)
				&& !dev->browsing)
			continue;
		if (interface && !strcmp(OBEX_PBAP_INTERFACE, interface)
				&& !dev->pbap)
			continue;
...
gboolean manager_allow_handsfree_connection(struct audio_device *device)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (handsfree_get_state(dev) > HANDSFREE_STATE_DISCONNECTED)
			connected++;

		if (connected >= max_connected_handsfree)
			return FALSE;
	}
...
gboolean manager_allow_pbap_connection(struct audio_device *device)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (pbap_get_state(dev) > PBAP_STATE_DISCONNECTED)
			connected++;

		if (connected >= max_connected_pbap)
			return FALSE;
	}
...
gboolean manager_allow_a2dpad_connection(struct audio_device *device, const bdaddr_t *dst)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (connected >= max_connected_a2dpad)
			return FALSE;
	}
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (connected >= max_connected_avrcpad)
			return FALSE;
	}
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (connected >= max_connected_browsing)
			return FALSE;
	}
