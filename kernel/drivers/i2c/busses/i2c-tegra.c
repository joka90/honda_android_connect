...
static irqreturn_t tegra_i2c_isr(int irq, void *dev_id)
{
	...
	if (status & I2C_INT_PACKET_XFER_COMPLETE) {
		...
		spin_lock_irqsave(&i2c_dev->irq_lock, flags);
		if (!i2c_dev->irq_disabled) {
			i2c_dev->irq_disabled = 1;
			disable_irq_nosync(i2c_dev->irq);
		}
		spin_unlock_irqrestore(&i2c_dev->irq_lock, flags);
...
err:
	dev_dbg(i2c_dev->dev, "reg: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n",
	...
	/* Disabling irqs to prevent further isr invocations */
	spin_lock_irqsave(&i2c_dev->irq_lock, flags);
	if (!i2c_dev->irq_disabled) {
		i2c_dev->irq_disabled = 1;
		disable_irq_nosync(i2c_dev->irq);
	...
	struct i2c_msg *msg, enum msg_end_type end_state)
{
	...
	/* Enabling irq for the single message */
	BUG_ON(!i2c_dev->irq_disabled);
	i2c_dev->irq_disabled = 0;
	enable_irq(i2c_dev->irq);

	if(((struct tegra_i2c_platform_data*)i2c_dev->dev->platform_data)->adapter_nr == 3){
		int i;
		for(i = 0; i < 10; ++i){
			ret = wait_for_completion_timeout(&i2c_dev->msg_complete, msecs_to_jiffies(100));
			if(ret != 0) {
				ret += msecs_to_jiffies(100) * (9 - i);
				break;
			}
	...
	if (WARN_ON(ret == 0)) {
		/* Disabling irqs after the message in order to avoid spurious
		   interrupts */
		spin_lock_irqsave(&i2c_dev->irq_lock, flags);
		if (!i2c_dev->irq_disabled) {
			i2c_dev->irq_disabled = 1;
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	...
	/* Disabling interrupts which should be enabled
	 * only during message transfer
	 */
	i2c_dev->irq_disabled = 1;
	disable_irq_nosync(i2c_dev->irq);
