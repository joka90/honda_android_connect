...
/* FUJITSU TEN :2012-10-01 FTEN_G start */
#ifdef CONFIG_FTEN_G
static void do_handle_rx_pio(struct tegra_uart_port *t, int limit);
#else
static void do_handle_rx_pio(struct tegra_uart_port *t);
#endif
/* FUJITSU TEN :2012-10-01 FTEN_G end */
...
/* FUJITSU TEN :2012-10-01 FTEN_G start */
#ifdef CONFIG_FTEN_G
	/* remove dma_sync_single_for_device() to reduce latency. */
#else
...
/* FUJITSU TEN :2012-10-01 FTEN_G start */
		...
		dma_sync_single_for_cpu(t->uport.dev, req->dest_addr,
				req->bytes_transferred, DMA_FROM_DEVICE);
#else
		dma_sync_single_for_cpu(t->uport.dev, req->dest_addr,
...
/* FUJITSU TEN :2012-10-01 FTEN_G start */
#ifdef CONFIG_FTEN_G
	/* limit PIO transfer size to 3bytes or less. */
	do_handle_rx_pio(t, 3);
#else
	do_handle_rx_pio(t);
#endif
/* FUJITSU TEN :2012-10-01 FTEN_G end */
...
static void tegra_flush_buffer(struct uart_port *u)
{
	...
	/* Disable interrupts as we dont want any further transmits to happen */
	ier = t->ier_shadow;
	ier &= ~UART_IER_THRI;
	t->ier_shadow = ier;
	uart_writeb(t, ier, UART_IER);
