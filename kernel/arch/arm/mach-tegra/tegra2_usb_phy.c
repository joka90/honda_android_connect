...
#define   UTMIP_XCVR_LSBIAS_SEL_SETUP(x)	(((x) & 0x1) << 21)
#define   UTMIP_XCVR_SETUP_MSB(x)		(((x) & 0x7) << 22)
#define   UTMIP_XCVR_HSSLEW_MSB(x)		(((x) & 0x7f) << 25)
#define   UTMIP_XCVR_MAX_OFFSET		4
...
#define   FUSE_USB_CALIB_MSBS(x)	(((x) & 0x70) >> 4)
...
static void utmi_phy_xcvr_setup(struct tegra_usb_phy *phy)
{
	...
	if (tegra_spare_fuse(57) == 1)
		cfg->term_range_adj = FUSE_USB_CALIB_MSBS(val);
	else
		cfg->term_range_adj = 6;
	if (cfg->xcvr_use_fuses) {
		...
		phy->utmi_xcvr_setup = val;
	} else {
		phy->utmi_xcvr_setup = cfg->xcvr_setup;
	}
}
...
static int utmi_phy_pad_power_on(struct tegra_usb_phy *phy)
{
	...
	val |= UTMIP_SQUELCH_LEVEL_SETUP(cfg->squelch_level);
	val |= UTMIP_HS_DISCON_MSB_SETUP(cfg->hsdiscon_msb);
...
static int utmi_phy_power_on(struct tegra_usb_phy *phy)
{
	...
	val |= UTMIP_XCVR_LSBIAS_SEL_SETUP(config->xcvr_lsbias_sel);
...
#ifdef CONFIG_BASEBOARD_VAR_1
	ret = otg_io_write(phy->ulpi_vp, 0x70, 0x31);
	if (ret) {
		pr_err("%s: ulpi write failed\n", __func__);
		return ret;
	}
