...
static int board_id_iterator __initdata = 0;
...
static int __init board_id_setup(char *line)
{
	...
	if (board_id_iterator >= TEGRA_MAX_BOARDS) {
		printk(KERN_WARNING"WARNING: Number of Board Id's more than Max Boards defined\n");
		return 0;
	}
...
bool tegra_is_board(const char *bom, const char *project,
	...
	while (i < TEGRA_MAX_BOARDS && board_info_array[i].valid != 0) {
		...
		b = (bom == NULL ||
			 !strncmp(board_info_array[i].bom,
				   bom, MAX_BUFFER)) &&
			(project == NULL ||
			 !strncmp(board_info_array[i].project,
				   project, MAX_BUFFER)) &&
			(sku == NULL ||
			 !strncmp(board_info_array[i].sku,
				   sku, MAX_BUFFER)) &&
			(revision == NULL ||
			 !strncmp(board_info_array[i].revision,
				   revision, MAX_BUFFER)) &&
			(bom_version == NULL ||
