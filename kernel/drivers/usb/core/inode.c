...
static void usbfs_add_device(struct usb_device *dev)
{
	...
	printk(KERN_DEBUG "usbfs_add_device %03d 0x%X \n",dev->devnum, dev->usbfs_dentry);
...
static void usbfs_remove_device(struct usb_device *dev)
{
	if (dev->usbfs_dentry) {
		...
		/* FUJITSU TEN :2013-08-22 driverec add start */
		printk(KERN_DEBUG "usbfs_remove_device %03d 0x%X \n",dev->devnum, dev->usbfs_dentry);
		/* FUJITSU TEN :2013-08-22 driverec add end */
		dev->usbfs_dentry = NULL;
	} else {
		/* FUJITSU TEN :2013-08-22 driverec add start */
		printk(KERN_WARNING "Failed usbfs_remove_device %03d 0x%X \n",dev->devnum, dev->usbfs_dentry);
		/* FUJITSU TEN :2013-08-22 driverec add end */
	}
}
