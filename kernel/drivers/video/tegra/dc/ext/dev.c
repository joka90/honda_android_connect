...
/* 2013-03-19 FUJITSU TEN suppress warning start */
#if 0
	if (err < 0)
		dev_err(&ext->dc->ndev->dev,
				"Window atrributes are invalid.\n");
#endif
/* 2013-03-19 FUJITSU TEN suppress warning end */
...
static void tegra_dc_ext_flip_worker(struct work_struct *work)
{
		...
		list_for_each_entry(temp, &ext_win->timestamp_queue,
			...
			if (j == 0) {
				if (unlikely(temp != data))
					dev_err(&win->dc->ndev->dev, "work queue did NOT dequeue head!!!");
				else
					head_timestamp =
	...
	if (!skip_flip) {
		tegra_dc_update_windows(wins, nr_win, wait_for_vblank);
