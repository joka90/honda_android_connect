...
/* Source file ID */
#define BLUEZ_SRC_ID BLUEZ_SRC_ID_LIB_SDP_C
...
void sdp_set_info_attr_for_avp(sdp_record_t *rec, const char *name)
{
	sdp_data_t *seq = malloc(sizeof(sdp_data_t));
	if( seq == NULL ){
		return;
	}
	sdp_attr_add_new(rec, SDP_ATTR_SVCNAME_PRIMARY, SDP_TEXT_STR8, name);
	seq->dtd = SDP_TEXT_STR8;
	seq->unitSize = sizeof(uint8_t);
	sdp_attr_replace(rec, SDP_ATTR_PROVNAME_PRIMARY, seq);
}
...
int sdp_get_supp_repo(const sdp_record_t *rec)
{
	sdp_data_t *d, *curr;
	uint32_t len = 0;
	char *val;
	
	d = sdp_data_get(rec, SDP_ATTR_SUPPORTED_REPOSITORIES);
	if (d == NULL) {
		return -1;
	}
	
	val = sdp_data_value(d, &len);
...
int sdp_get_avrcp_supp_feat(const sdp_record_t *rec)
{
	sdp_data_t *sdpdata = NULL;
	
	sdpdata = sdp_data_get(rec, SDP_ATTR_SUPPORTED_FEATURES);
	
	if (sdpdata == NULL || sdpdata->dtd < SDP_UINT8 || sdpdata->dtd > SDP_UINT32)
	{
		return -1;
	}

	return sdpdata->val.uint16;
}