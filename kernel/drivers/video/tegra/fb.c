...
static void tegra_fb_flip_win(struct tegra_fb_info *tegra_fb)
{
	...
	tegra_dc_update_windows(&tegra_fb->win, 1, true);
...
int tegra_fb_update_var(struct tegra_fb_info *tegra_fb,
					struct tegra_dc_mode *mode)
{
	struct fb_info *info = tegra_fb->info;
	struct fb_var_screeninfo var = { };
	...
	var.xres = mode->h_active;
	var.yres = mode->v_active;
	var.xres_virtual = mode->h_active;
	var.yres_virtual = mode->v_active * 2;
	var.left_margin = mode->h_back_porch;
	var.right_margin = mode->h_front_porch;
	var.upper_margin = mode->v_back_porch;
	var.lower_margin = mode->v_front_porch;
	var.hsync_len = mode->h_sync_width;
	var.vsync_len = mode->v_sync_width;
	var.pixclock = KHZ2PICOS(mode->pclk / 1000);

	/* sets var and adds the new mode to the modelist */
	info->flags |= FBINFO_MISC_USEREVENT;
	var.activate = FB_ACTIVATE_NOW;
	ret = fb_set_var(info, &var);

	/* forces an activation of previously added new mode */
