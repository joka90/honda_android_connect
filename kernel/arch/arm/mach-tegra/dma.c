...
#endif
#ifdef CONFIG_TEGRA_NVAVP_AUDIO
			...
			struct tegra_dma_req *req, bool is_stop_dma)
{
	...
	if (is_stop_dma) {
		...
		resume_dma();
		if (status & STA_ISE_EOC) {
			pr_debug("Got Dma Int here clearing");
			writel(status, ch->addr + APB_DMA_CHAN_STA);
		}
	...
	struct tegra_dma_req *req)
{
	...
	if (start_dma) {
		...
		if ((ch->mode & TEGRA_DMA_MODE_CONTINUOUS_DOUBLE) &&
		    (hreq->buffer_status != TEGRA_DMA_REQ_BUF_STATUS_HALF_FULL))
			goto end;
...
static void handle_continuous_dbl_dma(struct tegra_dma_channel *ch)
{
	...
	if (req->buffer_status == TEGRA_DMA_REQ_BUF_STATUS_EMPTY) {
		...
		/*
		 * We were expecting ping but got pong interrupt
		 * After the pong interrupt, dma would have continued
		 * with the  current request/buffer. So, don't do
		 * anything in this interrupt and do all the processing
		 * in next ping interrupt.
		 */

		if (!is_dma_ping_complete) {
			pr_info("%s %d missed audio ping interrupt for channel %d\n",
				__func__, __LINE__, ch->id);
			return;
		}
	...
	if (req->buffer_status == TEGRA_DMA_REQ_BUF_STATUS_HALF_FULL) {
		...
		/*
		 * We were expecting pong but got ping interrupt.
		 * Don't release it and lets continue with current
		 * request it will re-synchronize in the next pong.
		 */

		if (is_dma_ping_complete) {
			pr_info("%s %d missed audio pong interrupt for channel %d\n",
				__func__, __LINE__, ch->id);
			return;
		}
