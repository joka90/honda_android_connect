...
#define WM8731_CPU_DAI_SYSCLK 12000000

struct tegra_vcm {
	struct tegra_asoc_utils_data util_data;
	struct tegra_vcm_platform_data *pdata;
};
...
/* FUJITSU TEN:2013-12-05 AUDIO modify start */
	/*Bug 1345093: To allow two daps to run at different sample rate*/
	/*We don't update plla_out clock if codec is slave and tegra is master*/
	if (!pdata->codec_info[codec_id].master) {
		err = tegra_asoc_utils_set_rate(&machine->util_data,
				  srate, mclk);
		if (err < 0) {
...
/* FUJITSU TEN:2013-12-05 AUDIO modify end */
	...
	/* On E1853, when WM8731 audio codec functions in USB mode,*/
	/* i2sclock needs to be set to a fixed value of 12 MHz.    */
	/* If not, audible clicks are heard during capture.	   */
	if (strstr((pdata->codec_info[codec_id].codec_name), "wm8731")) {
		err = snd_soc_dai_set_sysclk(cpu_dai, 0, WM8731_CPU_DAI_SYSCLK,
				SND_SOC_CLOCK_IN);
	}
	...
	if (pdata->codec_info[codec_id].i2s_format ==
		...
		if (strstr((pdata->codec_info[codec_id].codec_name), "ad193x")) {
			/* Power on the AD1937 chip so that arecord */
			/* works immediately after boot-up*/
			reg = snd_soc_read(codec_dai->codec, AD193X_DAC_CTRL0);
			if (reg & 1) {
				reg &= 0xFE;
				snd_soc_write(codec_dai->codec,
						AD193X_DAC_CTRL0, reg);
			}
...
static int tegra_vcm_hw_params_controller3(
                                struct snd_pcm_substream *substream,
                                struct snd_pcm_hw_params *params)
{
        return tegra_vcm_hw_params(substream, params, 2);
}
...
/* FUJITSU TEN:2012-11-22 AUDIO(11E) add start */
static struct snd_soc_ops tegra_vcm_ops_controller3 = {
        .hw_params = tegra_vcm_hw_params_controller3,
        .hw_free = tegra_hw_free,
};
/* FUJITSU TEN:2012-11-22 AUDIO(11E) add end */
