...
static int tegra_gpio_suspend(void)
{
	...
	for (b = 0; b < ARRAY_SIZE(tegra_gpio_banks); b++) {
		...
		for (p = 0; p < ARRAY_SIZE(bank->oe); p++) {
		    ...
		    /* FUJITSU TEN:2012-11-15 eMMC add start */
/* FUJITSU TEN:2012-12-19 eMMC del start */
//#if defined(CONFIG_FJFEAT_PRODUCT_ADA1M)
/* FUJITSU TEN:2012-12-19 eMMC del end */
			if(((gpio & 0xE0) == 0xE0) && ((gpio & 0x18) == 0x08)) {
				bank->out[p] = __raw_readl(GPIO_OUT(gpio)) & 0x88;
			} else {
				bank->out[p] = 0;
			}
/* FUJITSU TEN:2012-12-19 eMMC del start */
//#else
//		    /* FUJITSU TEN:2012-11-15 eMMC add end */
