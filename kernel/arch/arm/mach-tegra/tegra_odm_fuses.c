...
{
	u32 reg, fuse_val[2], timeout = 2000;
	...
	do {
		mdelay(1);
		reg = tegra_fuse_readl(FUSE_CTRL);
	} while (((reg & (0x1 << 30)) != SENSE_DONE) && --timeout);
	if (!timeout)
		pr_warn("timeout waiting for fuse burning to take effect\n");
}
...
int tegra_fuse_program(struct fuse_data *pgm_data, u32 flags)
{
	...
	if (fuse_odm_prod_mode() && (flags != FLAGS_ODMRSVD)) {
		pr_err("Non ODM reserved fuses cannot be burnt after ODM production"
			"mode/secure mode fuse is burnt");
		return -EPERM;
	}
