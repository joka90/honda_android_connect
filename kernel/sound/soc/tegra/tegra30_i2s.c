...
int tegra30_i2s_enable_preset_clk(struct snd_pcm_substream *substream,
			struct snd_pcm_hw_params *params,
			struct snd_soc_dai *dai)
{
	struct tegra30_i2s *i2s = snd_soc_dai_get_drvdata(dai);
	u32 val;
	int srate, i2sclock, bitcnt;
				...
				struct snd_soc_dai *dai)
{
	...
	if (i2s->use_preset_clock) {
		tegra30_i2s_enable_preset_clk(substream, params, dai);
	} else {
		if (i2s->reg_ctrl & TEGRA30_I2S_CTRL_MASTER_ENABLE) {
			i2sclock = srate * params_channels(params) *
				sample_size;
			...
			/* FUJITSU TEN:2012-09-05 AUDIO add-start */
			i2sclock *= 2; 
			/* FUJITSU TEN:2012-09-05 AUDIO add-end */
...
int tegra30_i2s_set_sysclk(struct snd_soc_dai *cpu_dai, int clk_id,
		unsigned int freq, int dir)
{
	...
	i2s->use_preset_clock = 0;

	/* Check if the desired value for i2s_clk cab be dervied */
	/* from clk_pll_a_out0. If not, use clk_m for the same.  */
	if (clk_get_rate(i2s->clk_pll_a_out0) % freq == 0) {
		parent = clk_get(NULL, "clk_pll_a_out0");
		ret = clk_set_parent(i2s->clk_i2s, i2s->clk_pll_a_out0);
		if (ret) {
		...
		parent = clk_get(NULL, "clk_m");
		ret = clk_set_parent(i2s->clk_i2s, i2s->clk_m);
		if (ret) {
	...
	if (ret) {
		pr_err("Can't set rate of clk_i2s clock\n");
		return ret;
	}

	/* Set use_preset_clock variable to 1 so that the clock
	   frequency value does not get over-written */
	i2s->use_preset_clock = 1;
	clk_put(parent);
...
static struct snd_soc_dai_ops tegra30_i2s_dai_ops = {
	...
	.set_sysclk	= tegra30_i2s_set_sysclk,
};
...
static __devinit int tegra30_i2s_platform_probe(struct platform_device *pdev)
{
	...
	i2s->clk_m = clk_get(&pdev->dev, "clk_m");
	if (IS_ERR(i2s->clk_m)) {
		dev_err(&pdev->dev, "Can't retrieve mclk\n");
		ret = PTR_ERR(i2s->clk_m);
		goto err_mclk_put;
	}
